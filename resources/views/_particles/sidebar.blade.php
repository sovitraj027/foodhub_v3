...
<div class="col-md-3 col-sm-5 col-xs-12 side-bar">
      ...
      <tbody>
        <tr>
          <td><a href="{{URL::to('delete_item/'.$cart_item->id)}}" class="remove_item"><i
                class="fa fa-minus-circle"></i></a> <strong>{{$cart_item->quantity}}x</strong> {{$cart_item->item_name}}
          </td>
  ...
  <div id="cart_box" class="categories">
    ...
    <ul>
      ...
      <li>
        <label><a href="{{route('getMenus',$category->category_slug)}}">{{$category->category_name}}</a></label>
