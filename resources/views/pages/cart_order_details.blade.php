        ...
        <div class="row">
            <div class="col-md-9 col-sm-7 col-xs-12">
                <div class="box_style_2" id="order_process">
                    ...
                    <form action="{{url('order_details')}}" method="POST" id="order_details" enctype="multipart/form-data" autocomplete="off">
                        @csrf
                        <div class="message">
                            {{--
            ...
            <div class="col-md-3 col-sm-5 col-xs-12 side-bar">
                <div id="cart_box">
                    ...
                    <table class="table_summary table">
                        <tbody>
                            @foreach (\App\Cart::where('user_id', Auth::id())->orderBy('id')->get() as $n => $cart_item)
                            <tr>
                                <td><a href="{{ URL::to('delete_item/' . $cart_item->id) }}" class="remove_item"><i class="fa fa-minus-circle"></i></a>
                                    <strong>{{ $cart_item->quantity }}x</strong> {{ $cart_item->item_name }}
                                </td>
                                <td><strong class="pull-right">{{ getcong('currency_symbol') }}{{ $cart_item->item_price
                                        }}</strong>
                                </td>
                            </tr>
                            @endforeach
                    ...
                    <hr>
                    <div class="container">
                    <div class="custom-control custom-switch mt-3 mb-4">
                        <input type="checkbox" class="custom-control-input" id="statusSwitch" name="status" value="1" {{ old('status')
                            ? 'checked' : '' }}>
                        <label class="custom-control-label" for="statusSwitch">Pay With Esewa</label>
                    </div>
                    </div>
            ...
            <div id="help" class="box_style_2">
                ...
                <a href="tel://{{ getcong_widgets('need_help_phone') }}" class="phone">{{
                    getcong_widgets('need_help_phone') }}</a>
